 ...
 */
package com.packetproxyhub.repository;
...
public class MockRepositoryTest {
    ...
    class アカウントに関するテスト {
        ...
        @Test
        public void アカウントを更新できる() throws Exception {
            Account account1 = Account.create(Mail.create("aaa@example.com"));
            repository.insertAccount(account1);
            Account account2 = Account.create(account1.getId(), Name.create(), Mail.create("bbb@example.com"), "");
            repository.updateAccount(account2);
            Account found = repository.getAccount(account1.getId());
            assertEquals(account2.getMail(), found.getMail());
        }
