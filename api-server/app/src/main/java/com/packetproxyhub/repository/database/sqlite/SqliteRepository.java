...
import com.packetproxyhub.repository.database.sqlite.dao.*;
...
public class SqliteRepository implements IRepository {
    ...
    private void createTables() {
        ...
        BinaryMapper binaryMapper= session.getMapper(BinaryMapper.class);
        ...
        binaryMapper.createTable();
        snapshotMapper.createTable();
    ...
    public Ids listBinariesInConfig(Id configId) {
        ...
        try {
            BinaryMapper binaryMapper = session.getMapper(BinaryMapper.class);
            var daoBinarySet = binaryMapper.selectByConfigId(DaoId.create(configId));

            session.close();
            return Ids.createWithInit(daoBinarySet.stream().map(a -> a.getId().toId()).collect(Collectors.toSet()));
    ...
    public Binary getBinary(Id binaryId) {
        ...
        try {
            BinaryMapper binaryMapper = session.getMapper(BinaryMapper.class);
            DaoBinary binary = binaryMapper.select(DaoId.create(binaryId));

            session.close();
            if (binary == null) {
                return null;
            }
            return binary.toBinary();
    ...
    public Id insertBinaryToConfig(Id configId, Binary binary) {
        ...
        try {
            BinaryMapper binaryMapper = session.getMapper(BinaryMapper.class);
            binaryMapper.insert(new DaoBinary(configId, binary));
    ...
    public void removeBinaryFromConfig(Id configId, Id binaryId) {
        ...
        try {
            BinaryMapper binaryMapper = session.getMapper(BinaryMapper.class);
            binaryMapper.delete(DaoId.create(binaryId));
    ...
    public void updateBinary(Id configId, Binary binary) {
        ...
        try {
            BinaryMapper binaryMapper = session.getMapper(BinaryMapper.class);
            binaryMapper.update(new DaoBinary(configId, binary));
    ...
    public Ids listSnapshotsInConfig(Id configId) {
        ...
        try {
            SnapshotMapper snapshotMapper = session.getMapper(SnapshotMapper.class);
            var daoSnapshotSet = snapshotMapper.selectByConfigId(DaoId.create(configId));

            session.close();
            return Ids.createWithInit(daoSnapshotSet.stream().map(a -> a.getId().toId()).collect(Collectors.toSet()));
    ...
    public Snapshot getSnapshot(Id snapshotId) {
        ...
        try {
            SnapshotMapper snapshotMapper = session.getMapper(SnapshotMapper.class);
            DaoSnapshot snapshot = snapshotMapper.select(DaoId.create(snapshotId));
    ...
    public Id insertSnapshotToConfig(Id configId, Snapshot snapshot) {
        ...
        try {
            SnapshotMapper snapshotMapper = session.getMapper(SnapshotMapper.class);
            snapshotMapper.insert(new DaoSnapshot(configId, snapshot));
    ...
    public void removeSnapshotFromConfig(Id configId, Id snapshotId) {
        ...
        try {
            SnapshotMapper snapshotMapper = session.getMapper(SnapshotMapper.class);
            snapshotMapper.delete(DaoId.create(snapshotId));
    ...
    public void updateSnapshot(Id configId, Snapshot snapshot) {
        ...
        try {
            SnapshotMapper snapshotMapper = session.getMapper(SnapshotMapper.class);
            snapshotMapper.update(new DaoSnapshot(configId, snapshot));
